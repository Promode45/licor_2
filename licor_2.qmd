...
x <- "treatment"
cols <- 11:14 # the 4 continuous dependent variables
type <- "parametric" # given the large number of observations, we use the parametric version
...
mod3_exp <- lme(flux ~ treatment*ftime,
                random = ~ 1|rep/treatment,
               correlation = corExp(form = ~ftime),
                data = resp_final)
...
resp_final1 <- resp_final1 |> 
  mutate(date_time = if_else(time_of_day == "pm", date_time, as.POSIXct(date_time)+lubridate::hours(30)))
resp_final1 |> 
...
# Flux - drying cycle 3 
```{r}
event3_lp <- ggline(event3, x = "fdar", y = "flux", color = "treatment", add = "mean_se", facet.by = "time_of_day")+
  labs(x = "days after rain",
             y = "soil respiration",
...
event1_moisture <- ggline(event1, x = "fdar", y = "swc", color = "treatment", add = "mean_se")+
 ...
 pairwise_comparison <-  
  pairwise.wilcox.test(x = event1$swc, g = event1$treatment, p.adjust.method = "bonferroni")
pairwise_comparison$p.value
```
...
```
```{r}
redat <- make.rm(constant = c("label"),
                 repeated = "flux", data = resp_data_w)
```
